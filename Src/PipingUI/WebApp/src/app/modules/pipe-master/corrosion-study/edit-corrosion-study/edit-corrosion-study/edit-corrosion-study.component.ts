...
@Component({
  selector: 'app-edit-corrosion-study',
  templateUrl: './edit-corrosion-study.component.html',
  styleUrls: ['./edit-corrosion-study.component.css'],
  encapsulation: ViewEncapsulation.None
})
export class EditCorrosionStudyComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.route.queryParams.subscribe((params) => {
      this.editCorrosion = params.loopNo;
    });
    if (this.editCorrosion) {
      this.getCorrosionStudy();
    } else {
  ...
  getCorrosionStudy() {
    this.corrosionStudyService.getCorrosionData(this.editCorrosion).subscribe((res: any) => {
      this.clusterData = res;
      this.corrosionData = this.clusterData.corrosionStudyDTO;
      this.cofData = this.clusterData.cofMasterDTOList;
      this.iowData = this.clusterData.iowdtoList;
  ...
  onAddCOF() {
    this.cof.controls.id.setValue(this.cofDataSource.data.length);
  ...
  onAddIOW() {
    this.iow.controls.id.setValue(this.iowDataSource.data.length);
  ...
  onAddCluster(clusterNo) {
    this.cluster.controls.dmGuideID.setValue(this.clusterDatasource.data.length);
  ...
  onSubmit() {
    ...
    formData.append('CorrosionStudyInfo', JSON.stringify(corrosionData));
    if(this.editCorrosion) {
      this.corrosionStudyService.updateCorrosionStudy(formData).subscribe((res: any) => {
        console.log(res);
        this.snackBar.open('Successfully updated ' + res.loopNo, '', {
          duration: 5000,
        });
      })
    } else {
      this.corrosionStudyService.addCorrosionStudy(formData).subscribe((res) => {
